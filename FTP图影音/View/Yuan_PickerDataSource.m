...
- (void) dataSourceHourMinute {
    ...
    if (selectYears == _nowYear && ( selectMonths == _nowMonth || selectMonths > _nowMonth) && selectDays == _nowDay) {
    ...
    for (int i = 0 ; i < 24; i++) {
        [_hour addObject:[NSString stringWithFormat:@"%d",i]];
    }
    
    
    for (int i = 0; i < 60; i++) {
        
        [_minute addObject:[NSString stringWithFormat:@"%d",i]];
    }
